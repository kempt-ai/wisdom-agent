...
def migrate_add_resource_credibility_fields(connection, use_postgres: bool = True):
    """Add credibility assessment columns to knowledge_resources (Phase 7)."""
    from sqlalchemy import text

    columns_to_add = [
        ("credibility_verdict", "VARCHAR(50)"),
        ...
        try:
            if use_postgres:
                connection.execute(text(
                    f"ALTER TABLE knowledge_resources ADD COLUMN IF NOT EXISTS {col_name} {col_type}"
                ))
            else:
                result = connection.execute(text("PRAGMA table_info(knowledge_resources)")).fetchall()
                existing = [row[1] for row in result]
                if col_name not in existing:
                    connection.execute(text(
                        f"ALTER TABLE knowledge_resources ADD COLUMN {col_name} {col_type}"
                    ))
            connection.commit()
        except Exception as e:
            try:
            ...
            if 'already exists' not in str(e).lower() and 'duplicate' not in str(e).lower():
                print(f"Warning: Could not add {col_name} to knowledge_resources: {e}")

    print("Credibility fields added/verified on knowledge_resources")
