...
class FactCheckService:
    ...
    async def _check_single_claim(
        ...
        3. If no results or error, try ClaimBuster database
        4. If still no results or error, use LLM verification
        5. If all else fails, use direct LLM fallback
        
        FIXED 2025-01-08: Added try/except around each provider call to ensure
        fallback to LLM when external APIs fail with errors (not just empty results).
        """
        ...
        if google_provider and await google_provider.is_available():
            try:
                logger.debug(f"Trying Google Fact Check for: {claim_text[:50]}...")
                ...
                if result.get("results"):
                    ...
                    if external_matches or verdict not in ["unverifiable", None]:
                        logger.info(f"Google Fact Check found matches, verdict: {verdict}")
                        return {
                            "verdict": verdict,
                ...
                logger.warning(f"Google Fact Check failed with error: {e}, trying other providers")
        ...
        if claimbuster_provider and await claimbuster_provider.is_available():
            try:
                logger.debug(f"Trying ClaimBuster for: {claim_text[:50]}...")
                result = await registry.check_claim(
                    claim_text,
                ...
                if result.get("results"):
                    ...
                    if external_matches or verdict not in ["unverifiable", None]:
                        logger.info(f"ClaimBuster found matches, verdict: {verdict}")
                        return {
                            "verdict": verdict,
                            "confidence": result.get("confidence", 0.5),
                            "explanation": self._extract_explanation(result),
                ...
                logger.warning(f"ClaimBuster failed with error: {e}, trying LLM verification")
        ...
        if llm_provider and await llm_provider.is_available():
            try:
                logger.info(f"Using LLM verification for: {claim_text[:50]}...")
                result = await registry.check_claim(
                    claim_text,
                ...
                logger.warning(f"LLM verification provider failed: {e}, using direct fallback")
        
        # Final fallback: direct LLM check (this should always work if LLM router is configured)
        logger.info("All providers failed, using direct LLM fallback")
        return await self._llm_fallback_check(claim_text)
